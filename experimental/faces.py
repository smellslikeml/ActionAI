...
class FaceDetector:
    ...
    def run_inference(self, img):
        ...
        self.objs = self.detector.detectMultiScale(
            gray, scaleFactor=self.scale, minNeighbors=self.nbs, minSize=self.min_size
        )
        return
    ...
    def process_frame(self, img):
        ...
        for idx, (x, y, w, h) in enumerate(self.objs):
            ...
            img_copy[
                idx * self.inset : (idx + 1) * self.inset, : self.inset, :
            ] = cv2.resize(img[y : y + h, x : x + w, :], (self.inset, self.inset))
        return img_copy
