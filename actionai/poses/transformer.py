...
class PoseExtractor(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        for row in X:
            ...
            image = cv2.resize(image, (self.input_details[0]['shape'][1],self.input_details[0]['shape'][2]), interpolation=cv2.INTER_AREA)
            image = np.expand_dims(image.astype(self.input_details[0]['dtype'])[:, :, :3], axis=0)
...
if __name__ == '__main__':
    ...
    image = cv2.imread('/home/funk/human_stand.jpeg')
